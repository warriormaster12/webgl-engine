...
impl Engine {
    pub async fn new(window: &Window, init: fn()) -> Engine {
        ...
        // Global buffers 
        {
            ...
            context.create_buffer("camera_buffer", mem::size_of::<[f32; 16]>() as u64, wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST, None);
            context.create_buffer("material_buffer", mem::size_of::<context::GPUMaterialData>() as u64, wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST, None);

        }
        init(); // might initialize game items;
        Engine { context: context, meshes: Vec::new(), resolution: resolution }
    }
